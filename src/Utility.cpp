...
using VaRes   = UtilityFunctionLibrary::ValidationResults;

xt::xarray<double> UtilityFunctionLibrary::convolve_valid(const xt::xarray<double>& f, const xt::xarray<double>& g)
{
...
VaRes UtilityFunctionLibrary::dnn_validate(
  std::unique_ptr<SpikeDNNet> dnn, VlTrMap folds, std::uint16_t nEpochs, std::uint16_t kPoints, double step)
{
  VaRes results;
...
double UtilityFunctionLibrary::mean_squared_error(xt::xarray<double> yTrue, xt::xarray<double> yPred)
{
...
double UtilityFunctionLibrary::mean_absolute_error(xt::xarray<double> yTrue, xt::xarray<double> yPred)
{
...
double UtilityFunctionLibrary::timeit(std::function<void(void)> foo, std::uint32_t count)
{
...
std::unique_ptr<SpikeDNNet> UtilityFunctionLibrary::make_dnn(
  double alpha, std::uint32_t dim, std::unique_ptr<AbstractActivation> act1, std::unique_ptr<AbstractActivation> act2,
  std::unordered_map<std::string, xt::xarray<double>>& kwargs)
{
  return std::make_unique<SpikeDNNet>(
    std::move(act1), std::move(act2), kwargs["W_1"], kwargs["W_2"], dim, kwargs["A"], kwargs["P"], kwargs["K_1"],
    kwargs["K_2"], alpha);
}

VlTrMap UtilityFunctionLibrary::prepare_dataset(std::string trainingDataImportRoot)
{
