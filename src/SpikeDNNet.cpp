...
SpikeDNNet::SpikeDNNet(
  ...
  std::unique_ptr<AbstractActivation> act_func_2, xt::xarray<double> _mat_W_1,
  xt::xarray<double> _mat_W_2, size_t dim, xt::xarray<double> _mat_A,
  xt::xarray<double> _mat_P, xt::xarray<double> _mat_K_1,
  xt::xarray<double> _mat_K_2) :
  afunc_1{std::move(act_func_1)},
  afunc_2{std::move(act_func_2)}, mat_A{_mat_A}, mat_P{_mat_P},
  mat_K_1{_mat_K_1}, mat_K_2{_mat_K_2}, init_mat_W_1{_mat_W_1},
  init_mat_W_2{_mat_W_2}, mat_dim{dim}
{}
...
xt::xarray<double>
SpikeDNNet::moving_average(xt::xarray<double> x, std::uint32_t w)
{
  return UtilityFunctionLibrary::convolveValid(x, xt::ones<double>({w})) /
...
xt::xarray<double> SpikeDNNet::fit(
  xt::xarray<double> vec_x, xt::xarray<double> vec_u, double step,
  std::uint32_t n_epochs, std::uint32_t k_points)
{
  ...
  this->array_hist_W_1 =
    xt::ones<double>({nt, this->mat_dim, 2u * this->mat_dim});
  this->array_hist_W_2 =
    xt::ones<double>({nt, this->mat_dim, 2u * this->mat_dim});

  this->neuron_1_hist = xt::ones<double>({nt, 2u * this->mat_dim, size_t(1)});
  this->neuron_2_hist =
    xt::ones<double>({nt, 2u * this->mat_dim, vec_u.shape(1)});
...
xt::xarray<double> SpikeDNNet::predict(
  xt::xarray<double> init_state, xt::xarray<double> vec_u, double step)
{
  auto nt = vec_u.shape(0);
  xt::xarray<double> vec_est =
    init_state * xt::ones<double>({nt, this->mat_dim});
  ...
  for(auto i = 0u; i < nt - 1; ++i) {
    ...
    vec_est_next =
      cur_est +
      step * (xt::squeeze(xt::linalg::dot(this->mat_A, cur_est)) +
