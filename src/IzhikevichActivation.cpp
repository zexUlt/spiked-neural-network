...
IzhikevichActivation::IzhikevichActivation(
  double i_scale, double o_scale, double _izh_border, double a, double b,
  double c, double d, double e, std::vector<size_t> _shape) :
  input_scale{i_scale},
  output_scale{o_scale}, izh_border{_izh_border}, param_a{a}, param_b{b},
  param_c{c}, param_d{d}, param_e{e}, shape{_shape}
{
...
xt::xarray<double>
IzhikevichActivation::operator()(xt::xarray<double> input, double step = .01)
{
  xt::xarray<double> vec_scale = xt::ones<double>(this->shape);
  xt::xarray<double> cur_input = xt::broadcast(
    input.reshape({this->shape[0] / 2, 1}) * this->input_scale,
    {this->shape[0] / 2, this->shape[1]});
  ...
  this->state += step / 2 *
                 (.04 * this->state * this->state + 5. * this->state + 140. -
                  this->control + cur_input);
  ...
  this->control +=
    step * (this->param_a * (this->param_b * this->state - this->control));
